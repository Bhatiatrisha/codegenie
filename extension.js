...
function activate(context) {
	console.log('VanshGPT extension is now active!');

	context.subscriptions.push(
		vscode.commands.registerCommand('vanshgpt.configureLLM', configureLLM),
		vscode.commands.registerCommand('vanshgpt.getCodeSuggestions', getCodeSuggestions)
	);
}
...
async function configureLLM() {
	...
	if (provider && apiKey) {
		await vscode.workspace.getConfiguration().update('vanshgpt.llmProvider', provider, vscode.ConfigurationTarget.Global);
		await vscode.workspace.getConfiguration().update('vanshgpt.apiKey', apiKey, vscode.ConfigurationTarget.Global);
...
async function getCodeSuggestions() {
	const provider = vscode.workspace.getConfiguration().get('vanshgpt.llmProvider');
	const apiKey = vscode.workspace.getConfiguration().get('vanshgpt.apiKey');
